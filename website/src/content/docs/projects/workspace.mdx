...
`dxcl init` gives you an ordinary Vite project. There's nothing magic about it —
you
...
`activePreset` picks which one applies. Presets are how you have
...
The split is deliberate. `types.ts` is the contract, `preview-data.ts` is one
instance of it, `document.tsx` is structure only, and each node is a small module
under `nodes/`. Documents get long, and a template with all its text written
inline becomes unreadable quickly.
