...
A section takes its options and its children — node components, the same values
...
Nothing about a node changes because it moved inside one: it resolves the same
...
- An empty section renders as a lone heading. If the children come from a filter
  that can empty out, check that upstream.
