...
The text is the element's children, interpolated the way any JSX children are.
There is no template language, so formatting, branching and pluralisation are
...
A document that says one of three things is one component with three outcomes.
...
Distinct ids are what let the resolved document record which arm this recipient
