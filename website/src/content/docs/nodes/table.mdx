...
A width you give is a width you get. The table is packed with a fixed layout, so
Word lays the columns out where you put them rather than resizing them around
whatever happens to be in the cells — which is what makes a money column line up
with the total under it. That is checked rather than assumed: the conformance
suite packs this table, opens it in Word, and holds every column boundary to a
millimetre of where the preview drew it.
...
The preview repeats it too. A table longer than a page is broken between two of
its rows — the same seam Word breaks at — and the heading is drawn again at the
top of each piece, so a fifty-line invoice looks on screen the way it prints.
